...
struct WeatherModel {
    ...
    // forecasts every three hour
    let forecasts: [HourlyWeather]
    init(aLocation: String, aiconText: String, atemperature: String, aforecasts: [HourlyWeather], alat: Double, alon: Double, des: String) {
        location = aLocation
        iconText = aiconText
...
struct Temprature {
    ...
    init(displayType: tempratureDisplay, inDegrees: Double) {
        switch displayType {
        case .F:
            degrees = String(round(((inDegrees - 273.15) * 1.8) + 32)) + "\u{f045}"
            break
        case .C:
            degrees = String(round(inDegrees - 273.15)) + "\u{f03c}"
            break;
